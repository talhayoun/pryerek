    ...
    return (
        <div id="cart-wrap">
            ...
            <div id="cart" className={`${cartTog && !orderFormTog ? "active" : ""}`}>
                <h3 id="cart-title">
                    <span id="cart-close-icon" onClick={() => setCartTog(!cartTog)}>
                        <AiOutlineClose />
                    </span>{" "}
                    סל הקניות שלכם
                </h3>
                ...
                <div id="cart-items">
                    ...
                    <p className={`${cartItems.length ? "hide" : ""}`}>
                        העגלה ריקה, אולי תוסיפו קצת ירקות?
                    </p>
                </div>

                <div id="cart-sum">
                    {cartSumBeforeDiscounts != cartSum && (
                        <div>
                            <strong>סכום לפני הנחה: </strong> {cartSumBeforeDiscounts}₪
                        </div>
                    )}
                    <div>
                ...
                <div
                    id="cart-link-to-checkout"
                    onClick={() => setOrderFormTog(!orderFormTog)}
                >
                    להמשך הזמנה
                </div>
            </div>
